"""
run_tinyllama_trajectory.py — TinyLlama Training Trajectory Visualization (MMSP Method A).

Downloads TinyLlama checkpoint series and runs Trajectory-PCA to visualize
training evolution on a shared 2D plane.

Usage:
    python run_tinyllama_trajectory.py --gpu 1
"""
...
def load_tinyllama_checkpoint(model_name, device='cpu'):
    """Load a TinyLlama checkpoint and extract parameters."""
...
def main():
    ...
    print("=" * 70)
    print("TinyLlama Training Trajectory Visualization (MMSP Method A)")
    print("=" * 70)
    t_start = time.time()

    # TinyLlama checkpoint series (ordered by training step)
    checkpoints = [
        ("TinyLlama/TinyLlama-1.1B-step-50K-105b", "50K steps (105B tokens)"),
        ("TinyLlama/TinyLlama-1.1B-intermediate-step-240k-503b", "240K steps (503B tokens)"),
        ("TinyLlama/TinyLlama-1.1B-intermediate-step-480k-1T", "480K steps (1T tokens)"),
        ("TinyLlama/TinyLlama-1.1B-intermediate-step-715k-1.5T", "715K steps (1.5T tokens)"),
        ("TinyLlama/TinyLlama-1.1B-intermediate-step-955k-token-2T", "955K steps (2T tokens)"),
    ]
    ...
    for ckpt_name, label in checkpoints:
        model, params = load_tinyllama_checkpoint(ckpt_name)
    ...
    # Load the last checkpoint model for TADN reference
    last_model_name = checkpoints[-1][0]
    from transformers import AutoModelForCausalLM
    tadn_model = AutoModelForCausalLM.from_pretrained(
        last_model_name, torch_dtype=torch.bfloat16, trust_remote_code=True,
        attn_implementation="eager",
    )
    tadn_model.eval()
    ...
    checkpoint_losses = []
    for i, (ckpt_name, label) in enumerate(checkpoints):
        # Load checkpoint, set its params on centroid_model
        _, ckpt_params = load_tinyllama_checkpoint(ckpt_name)
        for name, param in centroid_model.named_parameters():
    ...
    print(format_metrics_table(metrics, "TinyLlama Trajectory PCA"))

    # --- Step 8: Visualization ---
    print("\nStep 8: Creating visualizations...")

    # 8a: Contour plot with checkpoint trajectories
    fig = plt.figure(figsize=(18, 7))

    # Left: Contour with trajectory
    ...
    ax1.set_title('TinyLlama Training Trajectory\n(Trajectory-PCA, MMSP Method A)')
    ...
    for (x, y) in projected_coords:
        # Find nearest grid point for z value
    ...
    ax1.set_title('TinyLlama Loss Evolution')
    ...
    results = {
        'experiment': 'TinyLlama Training Trajectory (MMSP Method A)',
    ...
    print("=" * 70)
    print("TinyLlama Trajectory Experiment Complete!")
    print("=" * 70)
